...
describe Nexmo::JWTBuilder do
  let(:decoded_token) { JWT.decode(builder.jwt.generate, nil, nil, { algorithm: 'RS256' }).first }

  context 'without custom configuration block' do
    let(:builder) do
      Nexmo::JWTBuilder.new(application_id: '123456789', private_key: './spec/nexmo_jwt/private_key.txt')
    end
  ...
  context 'with custom configuration block' do
    let(:builder) do
      Nexmo::JWTBuilder.new(
        application_id: '123456789',
        private_key: './spec/nexmo_jwt/private_key.txt',
        subject: 'ExampleApp',
        paths: {
          'acl' => {
            'paths' => {
    ...
    it 'generates a JWT string with the data provided' do
      ...
      expect(decoded_token['acl'])
        .to eql('paths' => { '/messages' => { 'methods' => %w[POST GET], 'filters' => { 'from' => '447977271009' } } })
  ...
  context 'without exp parameter provided in configuration block' do
    ...
    it 'sets exp parameter in JWT class to equal the value of the ttl and iat parameters added together' do
      builder.jwt.iat = 1_595_222_754
      hash = builder.jwt.to_payload
      expect(hash[:exp]).to eql(1_595_222_754 + 900)
    end
  end
  ...
  context 'with incorrect data types for custom configuration block' do
    it 'raises an ArgumentError if nbf parameter is not an Integer' do
      ...
      end.to raise_error(ArgumentError, "Expected Integer parameter type for NotBefore 'nbf' parameter")
    end
    ...
    it 'raises an ArgumentError if ttl parameter is not an Integer' do
      ...
      end.to raise_error(ArgumentError, "Expected Integer parameter type for TimeToLive 'ttl' parameter")
    end
    ...
    it 'raises an ArgumentError if acl_paths parameter is not a Hash' do
      ...
      end.to raise_error(ArgumentError, "Expected Hash parameter type for Paths 'paths' parameter")
    end
    ...
    it 'raises an ArgumentError if sub parameter is not a String' do
      ...
      end.to raise_error(ArgumentError, "Expected String parameter type for Subject 'subject' parameter")
    end
  end
  ...
  context 'with missing application_id parameter' do
    it 'raises a custom exception if Nil provided' do
      expect do
        Nexmo::JWTBuilder.new(application_id: nil, private_key: './spec/nexmo_jwt/private_key.txt')
      end.to raise_error(ArgumentError, "Missing required 'application_id' parameter")
    end

    it 'raises an exception of wrong number of parameters if not specified at all' do
      expect do
        Nexmo::JWTBuilder.new(private_key: './spec/nexmo_jwt/private_key.txt')
      end.to raise_error(KeyError, 'key not found: :application_id')
    end
  end
  ...
  context 'with missing private_key parameter' do
    it 'raises a custom exception if Nil provided' do
      expect do
        Nexmo::JWTBuilder.new(application_id: '123456789', private_key: nil)
      end.to raise_error(ArgumentError, "Missing required 'private_key' parameter")
    end

    it 'raises an exception of wrong number of parameters if not specified at all' do
      expect do
        Nexmo::JWTBuilder.new(application_id: '123456789')
      end.to raise_error(KeyError, 'key not found: :private_key')
    end
  end
